...
#include "Assets/Scripts/Utils/Lighting/BRDF.hlsl"
...
v2f initVert(appdata_tan v, float3 size, float3 rot, float3 pos){
    v2f o;

    v.vertex = sizeRotPos(v.vertex, size, rot, pos);
    o.vertex   = UnityObjectToClipPos(v.vertex);
    o.vertexW  = mul(unity_ObjectToWorld, v.vertex);
    o.normal   = UnityObjectToWorldNormal(rotCoord(v.normal.xyzz, rot));

    o.tangent  = UnityObjectToWorldDir(rotCoord(v.tangent, rot));
...
fixed4 shadedColor(v2f i){
    ...
    fixed4 baseColor     = i.color * tex2D(_MainTex, uv);
    ...
    ambientColor = lerp(diffuseColorI, specularColorI, _Metalness);
    ambientColor.rgb += ambient * _SpecularColor * fTerm * (1 - _Roughness);
    ambientColor.rgb *= pow(tex2D(_OcclusionTex, i.uv), _Occlusion); 

    #endif

    fixed4 color = lerp(diffuseColor, specularColor, _Metalness) + ambientColor;
    return color;
}

/*fixed4 frag(v2f i) : SV_Target
{
    return shadedColor(i);
}*/

#endif // HEIGHTMAP_INCLUDED